...
class AttendanceScreen(Screen):
    ...
    def show_leave_tab(self):
        ...
        # ========== ردیف اول: نوع مرخصی و میزان (وسط‌چین) ==========
        ...
        # ========== ردیف دوم: تاریخ شروع و پایان (وسط‌چین) ==========
        ...
        # ========== ردیف سوم: سقف مرخصی سال و داینامیک ==========
        ...
        # ========== ردیف پنجم: آمار مرخصی ==========
        stats_layout = BoxLayout(size_hint_y=None, height=dp(60), spacing=dp(8), padding=[dp(8), dp(5), dp(8), dp(5)])
        ...
        self.used_leave_label = RTLLabel(
        ...
        self.remaining_leave_label = RTLLabel(
            text='مانده: ۰ روز',
            size_hint_x=0.33,
            font_size=sp(14),
            color=(0.2, 0.9, 0.2, 1)
        ...
        self.hourly_used_label = RTLLabel(
            text='ساعتی: ۰۰:۰۰',
            size_hint_x=0.34,
            font_size=sp(14),
            color=(0.8, 0.8, 0.2, 1)
        ...
        Clock.schedule_once(lambda dt: self._update_leave_stats(), 0.2)
    ...
    def _update_leave_stats(self):
        ...
        try:
            ...
            print(f"annual_limit: {annual_limit}, dynamic_limit: {dynamic_limit}")
            ...
            if hasattr(self, 'annual_limit_field') and self.annual_limit_field:
                ...
                print(f"annual_limit_field.text = {self.annual_limit_field.text}")
            else:
                print("⚠️ annual_limit_field وجود ندارد!")
            ...
            if hasattr(self, 'dynamic_limit_field') and self.dynamic_limit_field:
                ...
                print(f"dynamic_limit_field.text = {self.dynamic_limit_field.text}")
            else:
                print("⚠️ dynamic_limit_field وجود ندارد!")
            
            file_path = os.path.join(get_data_path(), 'leave_requests.json')
            
            if not os.path.exists(file_path):
                self.used_leave_label.text = 'استفاده شده: ۰ روز'
                self.remaining_leave_label.text = f'مانده: {dynamic_limit} روز'
                self.remaining_leave_label.color = (0.2, 0.9, 0.2, 1)
                self.hourly_used_label.text = 'ساعتی: ۰۰:۰۰'
            ...
            # نمایش
            self.used_leave_label.text = f'استفاده شده: {total_used_days:.1f} روز'
            self.remaining_leave_label.text = f'مانده: {remaining_days:.1f} روز'
            
            hours = int(total_hourly_hours)
            minutes = int((total_hourly_hours - hours) * 60)
            self.hourly_used_label.text = f'ساعتی: {hours:02d}:{minutes:02d}'
            
            # تغییر رنگ مانده
            if remaining_days <= 0:
                self.remaining_leave_label.color = (0.9, 0.2, 0.2, 1)  # قرمز
            elif remaining_days <= 5:
                self.remaining_leave_label.color = (0.9, 0.8, 0.2, 1)  # زرد
            else:
                self.remaining_leave_label.color = (0.2, 0.9, 0.2, 1)  # سبز
    ...
    def _calculate_dynamic_leave_limit(self):
        """
        محاسبه سقف مرخصی داینامیک بر اساس زمان گذشته از سال
        فرمول: (سقف سالانه / ۱۲) × تعداد ماه‌های گذشته ← گرد به پایین
        مثال: 26 روز / 12 = 2.17 × 4.5 ماه = 9.75 ← 9 روز
        """
        try:
            config = AttendanceManager.load_config()
            annual_limit = config.get('annual_leave_limit', 30)
            ...
            # اگر روز جاری کمتر از ۱۵ باشد، ماه جاری نصف محاسبه شود
            ...
            print(f"DEBUG: months_passed={months_passed}, dynamic_limit={dynamic_limit}")
            ...
            config = AttendanceManager.load_config()
            return config.get('annual_leave_limit', 30)
        ...
        try:
            ...
            if leave_type == 'ساعتی':
                ...
                try:
                    ...
                    # روش ساده با استفاده از get_jalali_month_days
    ...
    def _update_leave_request(self, req_id):
        ...
        try:
            ...
            if leave_type in ['استحقاقی', 'ساعتی']:
                # محاسبه مرخصی استفاده شده بدون احتساب درخواست جاری
                file_path = os.path.join(get_data_path(), 'leave_requests.json')
                with open(file_path, 'r', encoding='utf-8') as f:
            ...
            # پاک کردن فرم و ریست حالت ویرایش
            self._clear_leave_form()
    ...
    def _load_leave_requests(self):
        """بارگذاری لیست درخواست‌های مرخصی با فیلد و چک‌باکس"""
        try:
            ...
            self.selected_leave_id = None  # برای ذخیره ID انتخاب شده
            ...
            for idx, req in enumerate(requests, 1):
                ...
                # ✅ چک‌باکس انتخاب (فقط برای مرخصی‌های فعال)
                cb = CheckBox(
                    active=False,
                    size_hint_x=0.08,
                    size_hint_y=None,
    ...
    def _on_select_leave(self, instance, value):
        ...
        if value:
            ...
            if row and isinstance(row, BoxLayout):
                # غیرفعال کردن چک‌باکس‌های دیگر در container
    ...
    def _update_leave_stats(self):
        """به‌روزرسانی آمار مرخصی با نمایش سقف داینامیک"""
        try:
            # محاسبه سقف داینامیک
            dynamic_limit = self._calculate_dynamic_leave_limit()
            ...
            if not os.path.exists(file_path):
                self.used_leave_label.text = f'استفاده شده: ۰ روز'
                self.remaining_leave_label.text = f'مانده: {dynamic_limit} روز (سقف: {dynamic_limit})'
                self.remaining_leave_label.color = (0.2, 0.9, 0.2, 1)
                self.hourly_used_label.text = 'ساعتی: ۰۰:۰۰'
                return
            ...
            # نمایش
            self.used_leave_label.text = f'استفاده شده: {total_used_days:.1f} روز'
            self.remaining_leave_label.text = f'مانده: {remaining_days:.1f} روز (سقف: {dynamic_limit})'
            ...
            # تغییر رنگ مانده
            if remaining_days <= 0:
                self.remaining_leave_label.color = (0.9, 0.2, 0.2, 1)
            elif remaining_days <= 5:
                self.remaining_leave_label.color = (0.9, 0.8, 0.2, 1)
            else:
                self.remaining_leave_label.color = (0.2, 0.9, 0.2, 1)
    ...
    def _print_leave_form(self, instance):
        ...
        try:
            ...
            # بررسی اینکه مرخصی انتخاب شده باشد
    ...
    def show_mission_tab(self):
        ...
        # ✅ دکمه ماموریت‌های از دست رفته (منقضی)
    ...
    def _load_missions(self):
        ...
        try:
            ...
            for m in all_missions:
                ...
                # ماموریت‌های تعیین تکلیف شده یا غیرفعال به تاریخچه میرن
            ...
            # هدر با ستون‌های کامل (مشابه فایل اکسل)
    ...
    def _assign_selected_missions(self, instance):
        """تعیین تکلیف ماموریت‌های انتخاب شده"""
        all_missions = self._load_missions_from_file()
        ...
        content.add_widget(RTLLabel(
            text=f'{len(selected)} ماموریت انتخاب شده است',
            size_hint_y=None,
            height=dp(35),
            font_size=sp(16),
            color=(1, 1, 1, 1)
    ...
    def _show_history_dialog(self, instance):
        """نمایش دیالوگ تاریخچه ماموریت‌ها"""
        try:
            ...
            if not history_missions:
                ...
                header = BoxLayout(size_hint_y=None, height=dp(22), spacing=dp(1), size_hint_x=None, width=dp(550))
                headers = ['شناسه', 'نوع', 'وضعیت', 'تاریخ پایان', 'امتیاز']
                sizes = [0.15, 0.22, 0.25, 0.20, 0.18]
                ...
                for idx, mission in enumerate(history_missions, 1):
                    row = BoxLayout(size_hint_y=None, height=dp(28), spacing=dp(1), size_hint_x=None, width=dp(550))
                    with row.canvas.before:
                        Color(0.12, 0.12, 0.16, 1) if idx % 2 == 0 else Color(0.09, 0.09, 0.12, 1)
                        rect = Rectangle(pos=row.pos, size=row.size)
                        row.bind(pos=lambda i, v, r=rect: setattr(r, 'pos', v),
                                size=lambda i, v, r=rect: setattr(r, 'size', v))
    ...
    def _show_expired_missions_dialog(self, instance):
        ...
        try:
            ...
            # فقط عنوان در داخل دیالوگ (بدون توضیحات اضافی)
            if expired_missions:
                content.add_widget(RTLLabel(
    ...
    def _print_mission_form(self, instance):
        ...
        try:
            ...
            # ✅ پیدا کردن ماموریت‌های انتخاب شده (چک‌باکس تیک خورده)
            selected_missions = []
            for m in all_missions:
                agent_name = m.get('agent_name', '').strip()
            ...
            # ✅ بعد از چاپ، انتخاب ماموریت رو لغو کن
            for m in all_missions:
                if m.get('selected', False):
                    m['selected'] = False
            self._save_missions_to_file(all_missions)
