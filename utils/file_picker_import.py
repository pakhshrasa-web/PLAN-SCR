...
class ImportFilePicker(BoxLayout):
    ...
    def _copy_from_uri(self, uri_str):
        try:
            ...
            if not filename:
                import hashlib
                filename = f"excel_{hashlib.md5(uri_str.encode()).hexdigest()[:8]}.xlsx"
    ...
    def _get_filename_from_uri(self, uri_str):
        try:
            ...
            return filename if ext else None
    ...
    def _pick_file_desktop(self):
        try:
            ...
            popup = Popup(title='📂 انتخاب اکسل', content=content, size_hint=(0.92, 0.8), auto_dismiss=False)
